...
local function getCurrentSecs(info)
    ...
    if info.online and info.joinTick then t = t+(tick()-info.joinTick) end
    return t
end
...
local function buildContent(clan, isFinal)
    local L={} local function w(s) L[#L+1]=s end
    w(string.rep("═",72))
    ...
    w(string.rep("─",72))
    local ord={}
    for uid,info in pairs(clan.logs) do ord[#ord+1]={uid=uid,info=info} end
    ...
    if recStart then w("Duração sessão : "..fmtDur(tick()-recStart)) end
    w("Fim            : "..(isFinal and timeOnly() or "--"))
    w("Total logados  : "..(clan.total or 0).." membro(s)")
    w(string.rep("═",72))
    return table.concat(L,"\n").."\n"
end
local function saveClan(clan,isFinal) safeWrite(clan.file,buildContent(clan,isFinal)) end
local function saveAllClans(isFinal)
    for _,c in ipairs(clans) do if (c.total or 0)>0 then saveClan(c,isFinal) end end
end
...
local function parseFile(content)
    ...
    for line in content:gmatch("[^\n]+") do
        local t,name,uid,entry,dur=line:match("^%[(%d+:%d+:%d+)%] | (.-) | (%d+) | (%d+:%d+:%d+) | (.-)%s*[🟢🔴]?%s*$")
        if t and name and uid then
            ...
            if id then
                ...
                players[id]={displayName=name:match("^%s*(.-)%s*$"),entryTime=entry or t,
                    joinTick=nil,totalSeconds=secs,sessions=1,idx=idx,online=false}
            end
        end
    end
    return players,idx
...
local function logPlayerToClan(player, clan)
    ...
    if clan.logs[uid] then
        ...
        info.joinTick=tick(); info.online=true; info.sessions=(info.sessions or 1)+1
    else
        clan.total=(clan.total or 0)+1
        clan.logs[uid]={displayName=player.DisplayName,entryTime=timeOnly(),
            joinTick=tick(),totalSeconds=0,sessions=1,idx=clan.total,online=true}
...
local function onLeave(player)
    ...
    for _,clan in ipairs(clans) do
        ...
        if info and info.online and info.joinTick then
            ...
            info.joinTick=nil; info.online=false
            saveClan(clan,false)
        end
    end
end
...
-- (forward refs para funções abaixo)

local function startRecording()
    if isRecording then return end
    if #clans==0 then return end
    ensureFolder(); isRecording=true; recStart=tick()
    for _,clan in ipairs(clans) do
        clan.file=clanFileName(clan)
        local ex=safeRead(clan.file)
        if ex and #ex>0 then
            local saved,n=parseFile(ex)
            clan.logs=saved; clan.total=n
            -- restaura configs de agendamento do primeiro arquivo encontrado
...
-- ══════════════════════════════════════════════════════════════
local CLAN_COLORS={
    Color3.fromRGB(55,130,255),Color3.fromRGB(80,200,100),Color3.fromRGB(220,100,60),
    Color3.fromRGB(180,80,220),Color3.fromRGB(220,200,50),Color3.fromRGB(60,200,200),
...
local function stroke(p,col,th) local s=Instance.new("UIStroke");s.Color=col or Color3.fromRGB(80,120,255);s.Thickness=th or 1.5;s.Parent=p end
...
local function mkBox(par,x,y,w,h,def,ph)
    ...
    corner(b,6); stroke(b,Color3.fromRGB(55,65,120),1); return b
end

local function mkDiv(par,y)
    local d=Instance.new("Frame")
    d.Size=UDim2.new(1,-24,0,1); d.Position=UDim2.new(0,12,0,y)
    d.BackgroundColor3=Color3.fromRGB(36,36,62); d.BorderSizePixel=0; d.Parent=par
end

-- ══════════════════════════════════════════════════════════════
--  JANELA PRINCIPAL
--  Layout top-down com Y incremental — sem sobreposição garantida
--
--  Seção          | Y início | Altura | Y fim
--  ─────────────────────────────────────────
--  TitleBar       |    0     |   44   |   44
--  Status+Clock   |   50     |   22   |   72
--  Divider        |   78     |    1   |   79
--  ── ADICIONAR CLÃ ──
--  Título seção   |   86     |   18   |  104
--  Labels campos  |  108     |   14   |  122
--  Boxes linha 1  |  124     |   32   |  156
--    tagBox       |  124  x=12   w=116
--    prefixBox    |  124  x=136  w=100
--    suffixBox    |  124  x=244  w=100
--  addBtn         |  124  x=350  w=90  h=32
--  addInfo        |  160     |   16   |  176
--  Divider        |  180     |    1   |  181
--  ── ABAS CLÃS ──
--  Título seção   |  188     |   18   |  206
--  TabScroll      |  208     |   36   |  244
--  Divider        |  248     |    1   |  249
--  ── PAINEL CLÃ SELECIONADO ──
--  Nome + count   |  256     |   20   |  276
--  Arquivo path   |  278     |   14   |  292
--  removeBtn      |  256  x=right  h=20
--  tHead          |  296     |   22   |  318
--  Scroll lista   |  320     |  128   |  448
--  Divider        |  452     |    1   |  453
--  ── GRAVAR ──
--  recBtn         |  460     |   38   |  498
--  Divider        |  504     |    1   |  505
--  ── AGENDAMENTO ──
--  Título         |  512     |   16   |  528
--  Labels HH:MM   |  530     |   14   |  544
--  Boxes + autoBtn|  546     |   34   |  580
--    sH x=12  w=44
--    :  x=60  w=12
--    sM x=76  w=44
--    eH x=148 w=44
--    :  x=196 w=12
--    eM x=212 w=44
--    autoBtn x=268 w=118 h=34
--  schedInfo      |  584     |   16   |  600
--  ─── TOTAL HEIGHT = 604 ───
-- ══════════════════════════════════════════════════════════════
...
mkLabel(Main,350, 106, 90,14,"",              10,Color3.fromRGB(150,150,200))  -- espaço do botão

-- boxes + botão na mesma linha (y=122, h=32)
...
-- feedback (y=160, h=16)
addInfo = mkLabel(Main,12,160,WW-24,16,"💡 Prefixo e/ou sufixo. Pode deixar um vazio.",
    10,Color3.fromRGB(110,110,160))
...
TabScroll.Size=UDim2.new(1,-24,0,34); TabScroll.Position=UDim2.new(0,12,0,209)
...
-- header tabela (y=298, h=22)
local tHead=Instance.new("Frame")
tHead.Size=UDim2.new(1,-24,0,22); tHead.Position=UDim2.new(0,12,0,298)
tHead.BackgroundColor3=Color3.fromRGB(20,20,42); tHead.BorderSizePixel=0; tHead.Parent=Main
corner(tHead,4)
mkLabel(tHead,6,0, math.floor((WW-24)*0.44),22,"USUÁRIO",10,Color3.fromRGB(155,155,200),Enum.Font.GothamBold)
mkLabel(tHead,math.floor((WW-24)*0.44),0,62,22,"ENTRADA",10,Color3.fromRGB(155,155,200),Enum.Font.GothamBold)
mkLabel(tHead,(WW-24)-82,0,76,22,"TEMPO",10,Color3.fromRGB(155,155,200),Enum.Font.GothamBold,Enum.TextXAlignment.Right)

-- scroll lista (y=322, h=120)
local Scroll=Instance.new("ScrollingFrame")
Scroll.Size=UDim2.new(1,-24,0,120); Scroll.Position=UDim2.new(0,12,0,322)
...
Scroll.Parent=Main; corner(Scroll,6); stroke(Scroll,Color3.fromRGB(30,30,58),1)

local ListLayout=Instance.new("UIListLayout")
ListLayout.SortOrder=Enum.SortOrder.LayoutOrder; ListLayout.Padding=UDim.new(0,2); ListLayout.Parent=Scroll
local lPad=Instance.new("UIPadding")
lPad.PaddingTop=UDim.new(0,3); lPad.PaddingLeft=UDim.new(0,4); lPad.PaddingRight=UDim.new(0,4); lPad.Parent=Scroll

local rows={}
local colW = WW - 24 - 8  -- largura útil dentro do scroll
for i=1,16 do
    local row=Instance.new("Frame")
    row.Size=UDim2.new(1,-0,0,22); row.BorderSizePixel=0; row.LayoutOrder=i
...
-- labels (y=526)
mkLabel(Main,12, 526,100,14,"▶ Início ( HH : MM )",10,Color3.fromRGB(130,210,130))
mkLabel(Main,148,526,100,14,"⏹ Fim  ( HH : MM )",  10,Color3.fromRGB(210,130,130))

-- boxes e botão agendar na mesma linha (y=542, h=34)
...
local function refreshList()
    if not activeClan then for _,r in ipairs(rows) do r.frame.Visible=false end; return end
    local ord={}
    for uid,info in pairs(activeClan.logs) do ord[#ord+1]={uid=uid,info=info} end
    table.sort(ord,function(a,b) return (a.info.idx or 0)<(b.info.idx or 0) end)
...
local function selectClan(clan)
    ...
    for _,tb in ipairs(tabButtons) do
        if tb.clan==clan then
            ...
            local s=tb.btn:FindFirstChildOfClass("UIStroke"); if s then s:Destroy() end
        end
    end
    if clan then
        clanNameLbl.Text="📁 "..clan.tag; clanNameLbl.TextColor3=clan.color
        clanCountLbl.Text="👥 "..(clan.total or 0).." membros"
        clanFileLbl.Text=(clan.file~="" and clan.file or clanFileName(clan))
        removeBtn.Visible=true
    else
        clanNameLbl.Text="Selecione um clã acima"; clanNameLbl.TextColor3=Color3.fromRGB(160,160,200)
        clanCountLbl.Text=""; clanFileLbl.Text=""; removeBtn.Visible=false
    end
    refreshList()
end

local function rebuildTabs()
    for _,tb in ipairs(tabButtons) do tb.btn:Destroy() end; tabButtons={}
    ...
    for i,clan in ipairs(clans) do
        ...
        tb.LayoutOrder=i; tb.Parent=TabScroll; corner(tb,7)
        if activeClan==clan then
            local s=Instance.new("UIStroke",tb); s.Color=Color3.fromRGB(255,255,255); s.Thickness=1.5
        end
        local entry={btn=tb,clan=clan}; tabButtons[#tabButtons+1]=entry
...
addBtn.MouseButton1Click:Connect(function()
    ...
    if tag=="" then addInfo.Text="⚠ Digite um nome/tag!"; addInfo.TextColor3=Color3.fromRGB(255,100,100); return end
    if pre=="" and suf=="" then addInfo.Text="⚠ Preencha prefixo e/ou sufixo!"; addInfo.TextColor3=Color3.fromRGB(255,100,100); return end
    for _,c in ipairs(clans) do
        if c.tag:upper()==tag:upper() then addInfo.Text="⚠ Clã '"..tag.."' já existe!"; addInfo.TextColor3=Color3.fromRGB(255,180,50); return end
    end
    if #clans>=8 then addInfo.Text="⚠ Máximo 8 clãs."; addInfo.TextColor3=Color3.fromRGB(255,100,100); return end
...
removeBtn.MouseButton1Click:Connect(function()
    ...
    for i,c in ipairs(clans) do if c==activeClan then table.remove(clans,i); break end end
    activeClan=clans[1] or nil; selectClan(activeClan); rebuildTabs()
    addInfo.Text="🗑 Clã removido."; addInfo.TextColor3=Color3.fromRGB(200,200,200)
end)

recBtn.MouseButton1Click:Connect(function()
    if #clans==0 then addInfo.Text="⚠ Adicione pelo menos 1 clã!"; addInfo.TextColor3=Color3.fromRGB(255,100,100); return end
    if not isRecording then
        startRecording()
        recBtn.Text="⏹  PARAR GRAVAÇÃO  (todos os clãs)"; recBtn.BackgroundColor3=Color3.fromRGB(200,50,50)
        statusLbl.Text="🔴 Gravando..."; statusLbl.TextColor3=Color3.fromRGB(255,80,80)
    else
        stopRecording()
        recBtn.Text="⏺  INICIAR GRAVAÇÃO  (todos os clãs)"; recBtn.BackgroundColor3=Color3.fromRGB(35,165,70)
...
autoBtn.MouseButton1Click:Connect(function()
    if not autoEnabled then
        if #clans==0 then schedInfo.Text="⚠ Adicione clãs antes!"; schedInfo.TextColor3=Color3.fromRGB(255,100,100); return end
...
-- ══════════════════════════════════════════════════════════════
task.defer(function()
    -- tenta carregar clãs de arquivos existentes hoje na pasta
    ensureFolder()
...
task.spawn(function()
    ...
    while ScreenGui.Parent do
        ...
        if isRecording then
            ...
            if autoSaveTimer>=CFG.AUTOSAVE_IV then autoSaveTimer=0; saveAllClans(false) end
        ...
        if autoEnabled then
            ...
            if not autoStopFired and H==autoStopH and M==autoStopM then
                if isRecording then
                    ...
                    recBtn.Text="⏺  INICIAR GRAVAÇÃO  (todos os clãs)"; recBtn.BackgroundColor3=Color3.fromRGB(35,165,70)
                    local total=0; for _,c in ipairs(clans) do total=total+(c.total or 0) end
