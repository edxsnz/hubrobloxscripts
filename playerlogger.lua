...
local function fmtDur(s)
    ...
    elseif s < 3600 then
        ...
        return string.format("%dh %02dm %02ds", math.floor(s / 3600), math.floor((s % 3600) / 60), s % 60)
    end
end
...
local function buildContent(clan, isFinal)
    ...
    w(string.rep("─", 72))
    w(string.format("%-12s | %-26s | %-12s | %-10s | %s", "[HORA]", "[USUÁRIO]", "[USER ID]", "[ENTRADA]",
        "[TEMPO TOTAL]"))
    w(string.rep("─", 72))
    local ord = {}
    ...
    for _, e in ipairs(ord) do
        ...
        w(string.format("[%s] | %-26s | %-12s | %-10s | %s %s",
            ...
            info.entryTime, fmtDur(getCurrentSecs(info)), info.online and "🟢" or "🔴"))
    end
    w(string.rep("─", 72))
...
local function parseConfig(content)
    ...
    if sh then
        cfg.startH = tonumber(sh); cfg.startM = tonumber(sm) or 0
    end
    if eh then
        cfg.stopH = tonumber(eh); cfg.stopM = tonumber(em) or 0
    end
    return cfg
end
...
local function stopRecording()
    ...
    if playerAddedConn then
        playerAddedConn:Disconnect(); playerAddedConn = nil
    end
    if playerRemovedConn then
        playerRemovedConn:Disconnect(); playerRemovedConn = nil
    end
    saveAllClans(true)
end
...
local function stroke(p, col, th)
    local s = Instance.new("UIStroke"); s.Color = col or Color3.fromRGB(80, 120, 255); s.Thickness = th or 1.5; s.Parent =
        p
end
...
clockLbl  = mkLabel(Main, WW - 150, 50, 138, 22, "🕐 --:--:--", 12, Color3.fromRGB(130, 190, 255), Enum.Font.GothamBold,
...
-- ══════════════════════════════════════════════════════════════
clanNameLbl            = mkLabel(Main, 14, 257, WW - 160, 20, "Selecione um clã acima", 12, Color3.fromRGB(160, 160, 200),
    Enum.Font.GothamBold)
clanCountLbl           = mkLabel(Main, WW - 148, 257, 136, 20, "", 12, Color3.fromRGB(160, 255, 160),
...
corner(tHead, 4)
mkLabel(tHead, 6, 0, math.floor((WW - 24) * 0.44), 22, "USUÁRIO", 10, Color3.fromRGB(155, 155, 200), Enum.Font
    .GothamBold)
mkLabel(tHead, math.floor((WW - 24) * 0.44), 0, 62, 22, "ENTRADA", 10, Color3.fromRGB(155, 155, 200),
    Enum.Font.GothamBold)
mkLabel(tHead, (WW - 24) - 82, 0, 76, 22, "TEMPO", 10, Color3.fromRGB(155, 155, 200), Enum.Font.GothamBold,
    Enum.TextXAlignment.Right)
...
lPad.PaddingTop = UDim.new(0, 3); lPad.PaddingLeft = UDim.new(0, 4); lPad.PaddingRight = UDim.new(0, 4); lPad.Parent =
    Scroll
...
for i = 1, 16 do
    ...
    local dur   = mkLabel(row, colW - 80, 0, 78, 22, "", 10, Color3.fromRGB(255, 210, 80), Enum.Font.GothamBold,
        Enum.TextXAlignment.Right)
...
local function refreshList()
    if not activeClan then
        for _, r in ipairs(rows) do r.frame.Visible = false end; return
    end
    local ord = {}
...
removeBtn.MouseButton1Click:Connect(function()
    ...
    for i, c in ipairs(clans) do
        if c == activeClan then
            table.remove(clans, i); break
        end
    end
        ...
        if autoEnabled then
            ...
            if not autoStopFired and H == autoStopH and M == autoStopM then
                if isRecording then
                    ...
                    recBtn.Text = "⏺  INICIAR GRAVAÇÃO  (todos os clãs)"; recBtn.BackgroundColor3 = Color3.fromRGB(35,
                        165, 70)
